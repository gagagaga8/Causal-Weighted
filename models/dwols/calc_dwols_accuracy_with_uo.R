# dWOLS full version - accuracy calculation
# Using model coefficients from train_dwols_with_uo.R

cat("============================================================\n")
cat("dWOLS accuracy calculation (full version - with urine output features)\n")
cat("============================================================\n\n")

# Load model coefficients
cat("Step 1: Loading model coefficients...\n")
load("../data/dwols_model_with_uo.RData")
...
# get_mats_with_uoFunction - withTraining 
get_mats_with_uo <- function(d) {
  dat <- d
  
  # k=1: All Patient butneedtoFilterNA
  valid_k1 <- complete.cases(dat[, c("admission_age", "creat_k1", "pot_k1")])
  dat_k1 <- dat[valid_k1, ]
  ...
  valid_k2 <- complete.cases(dat_k2_pre[, c("sofa_24hours", "bun_k2", "ph_k2", "ph_k1", "uo_k2", "uo_k1")])
  ...
  valid_k3 <- complete.cases(dat_k3_pre[, c("uo_k3", "bun_k3", "bun_k1")])
...
calc_accuracy <- function(ite, actual_action, name) {
  ...
  # startclassAccuracy
  if(sum(actual_action == 1) > 0) {
    start_acc <- sum(recommended == 1 & actual_action == 1) / sum(actual_action == 1)
    cat(sprintf("    startclassAccuracy: %.2f%%\n", start_acc * 100))
  }
