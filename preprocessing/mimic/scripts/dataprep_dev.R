# ============================================
# MIMIC-IV data preprocessing script (adapted version)
# Original code based on MIMIC-III, now adapted for MIMIC-IV
# ============================================
...
# items1 fromPostgreSQLData use Table Table 
con <- create_mimic_connection()

# due to TablenotGenerate from TableExtract
# needto SQLQuery Table
# useCSVFile 
dbDisconnect(con)

# items2 fromCSVFile when use 
# MIMIC-IV usestay_id icustay_id
data_path <- "~/Desktop/TEAM METHODS/phd/Mimic/extracted_data/"

# CSVFilenot in scheme
if(!file.exists(paste0(data_path, "kdigo_stages.csv"))) {
  stop("Error CSVFilenot in \n below items \n1. MIMIC-III/IV TableCSVFile\n2. MIMIC-IVData and Run SQL")
}

pivoted_rrt <- read.csv(paste0(data_path, "pivoted_rrt.csv"))
crrt_durations <- read.csv(paste0(data_path, "crrt_durations.csv"))
kdigo_stages <- read.csv(paste0(data_path, "kdigo_stages.csv"))
icustay_detail <- read.csv(paste0(data_path, "icustay_detail.csv"))
pivoted_lab <- read.csv(paste0(data_path, "pivoted_lab.csv"))
pivoted_bg <- read.csv(paste0(data_path, "pivoted_bg.csv"))
esrd_patients <- read.csv(paste0(data_path, "esrd_patients.csv"))
vasopressor_durations <- read.csv(paste0(data_path, "vasopressor_durations.csv"))
ventilation_durations <- read.csv(paste0(data_path, "ventilation_durations.csv"))
admissions <- read.csv(paste0(data_path, "admissions.csv"))

# MIMIC-IIItoMIMIC-IV mapping if CSV MIMIC-IV 
# if DatasetPackage stay_idandnonicustay_id Executebelow Convert 
if("stay_id" %in% colnames(icustay_detail)) {
  icustay_detail$icustay_id <- icustay_detail$stay_id
  if("stay_id" %in% colnames(pivoted_rrt)) pivoted_rrt$icustay_id <- pivoted_rrt$stay_id
  if("stay_id" %in% colnames(kdigo_stages)) kdigo_stages$icustay_id <- kdigo_stages$stay_id
  if("stay_id" %in% colnames(vasopressor_durations)) vasopressor_durations$icustay_id <- vasopressor_durations$stay_id
  if("stay_id" %in% colnames(ventilation_durations)) ventilation_durations$icustay_id <- ventilation_durations$stay_id
}
...
weight_durations <- read.csv(paste0(data_path, "weight_durations.csv"))
...
#### Get baseline SOFAs
pivoted_sofa <- read.csv(paste0(data_path, "pivoted_sofa.csv"))
...
immunosuppressant <- read.csv(paste0(data_path, "immunosuppressant.csv"))
